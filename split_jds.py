...
df = pd.read_csv('download_this_dataset/jd.csv')
...
def categorize_row(row, column_name):
    ...
    # most appeared word
    max_count = max(intern_count, junior_count, senior_count, pt_count)
...
if __name__ == "__main__":
    ...
    df['category'] = df.apply(categorize_row, column_name='jobtitle', axis=1)
    ...
    intern_df.to_csv('download_this_dataset/separated_data/intern.csv', index=False)
    junior_df.to_csv('download_this_dataset/separated_data/junior.csv', index=False)
    senior_df.to_csv('download_this_dataset/separated_data/senior.csv', index=False)
    pt_df.to_csv('download_this_dataset/separated_data/pt.csv', index=False)